...
async function waitForProxyHealth(port, timeoutMs = 10000, intervalMs = 150) {
...
async function main() {
  ...
  if (proxy.shouldStart) {
    ...
    // Expo web starts serving the page immediately after this, and the app's
    // first API call can otherwise race the proxy still binding its port —
    // that race is what surfaced as "Unable to reach the local LAD API proxy"
    // even right after a clean `npm run web`. Block here until it actually
    // answers /__health (or give up after a timeout so a genuinely broken
    // proxy doesn't hang the whole dev startup).
    const ready = await waitForProxyHealth(authProxyPort);
    if (!ready) {
      console.warn(`Auth proxy on ${webApiUrl} did not report healthy within 10s — starting Expo web anyway.`);
    }
  } else {
