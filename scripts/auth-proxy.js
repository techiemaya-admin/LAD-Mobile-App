...
const dnsFallback = require('./dns-fallback');

// Must run before any outbound fetch so filtered/refusing network DNS
// (a common cause of `fetch failed` / ENOTFOUND on *.run.app hosts)
// falls back to public resolvers instead of failing the request.
dnsFallback.install();
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.error(
      '[auth-proxy] Error forwarding request:',
      req.method,
      req.url,
      error.message,
      causeCode ? `(${causeCode})` : '',
    ...
    if (isDnsFailure) {
      console.error(
        '[auth-proxy] DNS could not resolve the backend host. This is usually a filtered network resolver.',
        'Set DNS_FALLBACK_SERVERS (default 8.8.8.8,1.1.1.1) or switch this machine to a public DNS server.',
      );
    }
    ...
    writeJson(res, 502, headers, {
      ...
      message: isAbort
        ...
        : isDnsFailure
          ? 'DNS lookup for the backend host failed from this machine. Check the network DNS server or set DNS_FALLBACK_SERVERS.'
