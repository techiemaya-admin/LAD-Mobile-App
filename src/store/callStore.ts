...
const getCallLogsErrorMessage = (error: unknown, fallback: string) => {
...
export const useCallStore = create<CallState>((set, get) => ({
    ...
    fetchCallsInFlight = (async () => {
      ...
      try {
        ...
        set({
          ...
          isRefreshing: false,
          hasMore: false,
          error: getCallLogsErrorMessage(error, 'Unable to load real call logs from the backend.'),
        });
      }
    })();
  ...
  fetchNextCalls: async () => {
    ...
    try {
      ...
      set({
        isLoadingMore: false,
        error: getCallLogsErrorMessage(error, 'Unable to load more call logs.'),
      });
    }
  },
