...
interface ChatState {
  ...
  sendContactMessage: (name: string, phone: string, email: string, company?: string) => Promise<void>;
...
const getLatestMessagePreview = (message?: ChatMessage) => {
  if (!message) return '';
  const caption = message.mediaCaption?.trim();
  if (caption) return caption;

  const content = message.content?.trim();
  const contentIsBareMediaId = Boolean(content && /^\d{10,}$/.test(content));
  if (content && !contentIsBareMediaId) return content;
...
const reconcileConversationPreview = (
  conversations: Conversation[],
  conversationId: string,
  messages: ChatMessage[],
) => {
  const latestMessage = messages[messages.length - 1];
  if (!latestMessage) return markConversationReadLocally(conversations, conversationId);
  ...
  return conversations.map((conversation) =>
    conversation.id === conversationId
      ? {
          ...
          lastMessageAt: latestMessage.createdAt || conversation.lastMessageAt,
          status: latestMessage.status || conversation.status,
          unreadCount: 0,
        }
      : conversation
  );
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  initializeRealtime: () => {
    ...
    socket.on('connect_error', () => {
      // Realtime is an enhancement, not the source of truth for the initial
      // inbox. Keep successfully loaded REST conversations visible when the
      // socket service alone is temporarily unavailable.
      set((state) => state.conversations.length
        ? {}
    ...
    socket.on('notification:new', (payload) => {
      ...
      if (get().activeConversationId === conversationId && !isOwnMessage) {
        void get().markConversationRead(conversationId).catch(() => undefined);
      }
    });
  ...
  fetchConnectedIntegrations: async () => {
    ...
    try {
      ...
      set((state) => ({
        isLoadingIntegrations: false,
        syncError: state.conversations.length
          ? null
          : getErrorMessage(error, 'Unable to load connected integrations.'),
      }));
    }
  },
  ...
  setActiveConversation: async (conversationId, options = {}) => {
    ...
    if (hasCachedMessages && !options.force) {
      set((state) => ({
        conversations: reconcileConversationPreview(state.conversations, conversationId, cachedMessages),
      }));
      return;
    }
    ...
    try {
      ...
      set((state) => {
        const conversationsWithDetail = detail?.conversation?.id
          ? upsertConversation(state.conversations, {
              ...detail.conversation,
              unreadCount: localReadOverrides.has(conversationId) ? 0 : detail.conversation.unreadCount,
            })
          : state.conversations;

        return {
          conversations: reconcileConversationPreview(conversationsWithDetail, conversationId, sortedMessages),
  ...
  sendContactMessage: async (name, phone, email, company) => {
    const activeConversationId = get().activeConversationId;
    const contactName = name.trim();
    const contactPhone = phone.trim();
    const contactEmail = email.trim();
    if (!activeConversationId || !contactName || !contactPhone || !contactEmail) return;

    const clientId = `local-contact-${Date.now()}`;
    const optimisticMessage: ChatMessage = {
      id: clientId,
      conversationId: activeConversationId,
      content: contactName,
      sender: 'agent',
      channel: get().conversations.find((item) => item.id === activeConversationId)?.channel ?? 'whatsapp',
      status: 'sending',
      createdAt: new Date().toISOString(),
      type: 'contact',
      contactName,
    ...
    set((state) => ({
      ...
      conversations: state.conversations.map((conversation) => conversation.id === activeConversationId
        ? { ...conversation, lastMessage: `Contact: ${contactName}`, lastMessageAt: optimisticMessage.createdAt, status: 'sending' }
        : conversation),
    }));

    try {
      const currentUser = useAuthStore.getState().user;
      const returnedMessage = await sendChatMessage({
        conversationId: activeConversationId,
        message: '',
        content: '',
        type: 'contact',
      ...
      const sentMessage: ChatMessage = {
        ...optimisticMessage,
        ...returnedMessage,
        type: returnedMessage?.type || 'contact',
        contactName: returnedMessage?.contactName || contactName,
        contactPhone: returnedMessage?.contactPhone || contactPhone,
        contactEmail: returnedMessage?.contactEmail || contactEmail,
        contactCompany: returnedMessage?.contactCompany || company?.trim() || undefined,
        content: returnedMessage?.content || contactName,
        status: returnedMessage?.status ?? 'sent',
      };
      set((state) => ({
        isSending: false,
        activeMessages: state.activeMessages.map((message) => message.id === clientId ? sentMessage : message),
        messagesByConversationId: {
          ...state.messagesByConversationId,
          [activeConversationId]: (state.messagesByConversationId[activeConversationId] ?? state.activeMessages)
            .map((message) => message.id === clientId ? sentMessage : message),
        },
        conversations: state.conversations.map((conversation) => conversation.id === activeConversationId
      ...
      set((state) => ({
        isSending: false,
        activeMessages: state.activeMessages.map((message) => message.id === clientId ? { ...message, status: 'failed' } : message),
        messagesByConversationId: {
          ...state.messagesByConversationId,
          [activeConversationId]: (state.messagesByConversationId[activeConversationId] ?? state.activeMessages)
            .map((message) => message.id === clientId ? { ...message, status: 'failed' } : message),
        },
        error: getErrorMessage(error, 'Contact failed to send.'),
      }));
    }
  },
  ...
  markConversationRead: async (conversationId) => {
    ...
    // The override prevents a stale sync response from briefly restoring the dot;
    // the backend request below makes the read state durable across app restarts.
