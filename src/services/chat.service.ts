import {
  ...
  apiPost,
  buildApiUrl,
  expireAuthSessionForUnauthorizedRequest,
  getAuthToken,
  safeStorage,
  WEB_API_URL,
} from '@/src/api';
...
const directBackendRequest = async (
  ...
  if (!response.ok) {
    if (response.status === 401) {
      await expireAuthSessionForUnauthorizedRequest(path, token);
    }
...
const normalizeBniMessage = (item: ApiMessage | RawRecord, fallbackConversationId: string): ChatMessage => {
  ...
  return {
    ...
    contactName: item.contact_name ?? item.contactName ?? metadata.contact_name
      ? String(item.contact_name ?? item.contactName ?? metadata.contact_name)
      : undefined,
    contactPhone: item.contact_phone ?? item.contactPhone ?? metadata.contact_phone
      ? String(item.contact_phone ?? item.contactPhone ?? metadata.contact_phone)
      : undefined,
    contactEmail: item.contact_email ?? item.contactEmail ?? metadata.contact_email
      ? String(item.contact_email ?? item.contactEmail ?? metadata.contact_email)
      : undefined,
    contactCompany: item.contact_company ?? item.contactCompany ?? metadata.contact_company
      ? String(item.contact_company ?? item.contactCompany ?? metadata.contact_company)
      : undefined,
...
const normalizeLinkedInConversation = (item: RawRecord): ChatConversation | null => {
  ...
  const inlineMessages = getArrayPayload(item.messages ?? [], ['messages', 'data', 'items'])
    .filter(isRecord)
    .map((message) => normalizeLinkedInMessage(message, id))
    .filter((message) => message.content);
  const latestInlineMessage = inlineMessages[inlineMessages.length - 1];
  const lastMessage =
    item.last_message ??
    item.lastMessage ??
    item.latest_message ??
  ...
  const lastMessageText = typeof lastMessage === 'string'
    ? lastMessage
    : lastMessageRecord.content ?? lastMessageRecord.text ?? lastMessageRecord.body ?? lastMessageRecord.message ?? '';
  ...
  return {
    ...
    name: String(name),
    channel: 'linkedin',
    lastMessage: normalizeLastMessagePreview(String(lastMessageText || (state === 'pending' ? 'Connection request sent' : 'No messages yet'))),
    lastMessageAt: asDateString(
    ...
    messageCount: Number(item.message_count ?? item.messages_count ?? inlineMessages.length),
  };
};
...
class ChatService {
  ...
  async sendMessage({
    ...
    if (type === 'contact') {
      ...
      if (contactCompany) payload.contact_company = contactCompany;
    }
    ...
    if (bniConversationIds.has(conversationId)) {
      ...
      if (type === 'contact') {
        bniPayload.contact_name = contactName;
        bniPayload.contact_phone = contactPhone;
        if (contactEmail) bniPayload.contact_email = contactEmail;
        if (contactCompany) bniPayload.contact_company = contactCompany;
      }
    ...
    if (linkedinConversationIds.has(conversationId)) {
      ...
      if (type === 'contact') {
        linkedinPayload.contact_name = contactName;
        linkedinPayload.contact_phone = contactPhone;
        if (contactEmail) linkedinPayload.contact_email = contactEmail;
        if (contactCompany) linkedinPayload.contact_company = contactCompany;
      }
  ...
  async markAsRead(conversationId: string) {
    ...
    if (linkedinConversationIds.has(conversationId)) {
      await bniRequest('POST', `/api/conversations/${encodedId}/read`, undefined, { backendChannel: 'linkedin' });
      return;
    }
    ...
    if (
      ...
      await apiPost(`/api/conversations/${encodedId}/read`);
      return;
    }

    const backendChannel = await getConversationBackendChannel(conversationId);
    try {
      await bniRequest('POST', `/api/conversations/${encodedId}/read`, undefined, { backendChannel });
    } catch {
      // Older WABA/WAPA deployments persist read state while loading detail.
      await bniRequest('GET', `/api/conversations/${encodedId}`, undefined, { backendChannel });
    }
  }
