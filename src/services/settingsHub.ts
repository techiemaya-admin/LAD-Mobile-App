...
// ── Campaign detail / live-activity (mirrors LAD-Frontend-2's
// /campaigns/[id]/analytics page — used by the mobile campaign detail
// screen to drive the "Campaign Accelerator" step tracker). ─────────────────
...
export type CampaignAnalyticsOverview = {
  ...
  /** Raw step config from the backend — feeds buildDisplaySteps in campaignActivity.ts. */
  stepAnalytics: { id?: string; type: string; title?: string; order?: number; config?: unknown }[];
};
...
export async function getCampaignAnalyticsOverview(campaignId: string): Promise<CampaignAnalyticsOverview> {
  const response = await apiGet<unknown>(`/api/campaigns/${encodeURIComponent(campaignId)}/analytics`);
  const data = asRecord(unwrapData(response.data));
  const overview = asRecord(data.overview);
  const campaign = asRecord(data.campaign);
  const stepAnalytics = Array.isArray(data.step_analytics)
    ? (data.step_analytics as ApiRecord[]).map((step) => ({
      id: pickString(step.id) || undefined,
      type: pickString(step.type),
      title: pickString(step.title) || undefined,
      order: pickOptionalNumber(step.order) ?? undefined,
      config: step.config,
    }))
    : [];
  ...
  return {
    campaign: {
      id: pickString(campaign.id, campaignId),
      name: pickString(campaign.name, 'Untitled campaign'),
      status: pickString(campaign.status, 'draft'),
      createdAt: pickString(campaign.created_at) || undefined,
    },
    overview: {
      totalLeads: pickNumber(overview.total_leads),
      activeLeads: pickNumber(overview.active_leads),
      completedLeads: pickNumber(overview.completed_leads),
      stoppedLeads: pickNumber(overview.stopped_leads),
      sent: pickNumber(overview.sent),
      delivered: pickNumber(overview.delivered),
      opened: pickNumber(overview.opened),
      clicked: pickNumber(overview.clicked),
      connected: pickNumber(overview.connected),
      replied: pickNumber(overview.replied),
    },
    stepAnalytics,
  };
}
...
): Promise<{ activities: CampaignActivityRow[]; total: number }> {
  const response = await apiGet<unknown>(`/api/campaigns/${encodeURIComponent(campaignId)}/analytics`, {
    params: { limit: options.limit ?? 500, offset: options.offset ?? 0 },
  });
  const data = asRecord(unwrapData(response.data));
  const activities = Array.isArray(data.activities) ? data.activities as CampaignActivityRow[] : [];
  return {
    activities,
    total: pickNumber(data.total) || activities.length,
  };
}
