...
function mergeCountsWithBackend(contacts: CrmContact[], stats?: CRMStats | null, authoritativeTotal?: number) {
  const visibleCounts = buildCounts(contacts);
  const backendStageCounts = backendStageCountsFromStats(stats);
  const backendTotal = authoritativeTotal ?? toNumber(stats?.total_leads, visibleCounts.all);
  const backendCounts = countsFromStageCounts(backendStageCounts, backendTotal);

  return {
    // Use the same backend total as pagination instead of promoting a stale
    // stage/stat count above the API's canonical contact count.
    all: Math.max(0, backendTotal),
...
export async function listProspects(params: {
  ...
  try {
    ...
    const crmData = await fetchCRMData({
      page: Math.floor((params.offset ?? 0) / Math.max(limit, 1)) + 1,
      limit,
...
export async function fetchProspectCRMData(params: {
  limit?: number;
  offset?: number;
  search?: string;
  stage?: string;
  channel?: ChannelKey;
  assignedTo?: string;
} = {}): Promise<ProspectCRMData> {
  const limit = params.limit ?? 200;
  const offset = params.offset ?? 0;
  const search = params.search?.trim();
  const [prospectsResult, pipelineResult] = await Promise.allSettled([
    listProspects({ limit, offset, search, lifecycle_stage: params.stage as LifecycleStage | undefined, channel: params.channel }),
    fetchCRMData({
  ...
  const paginationTotal = pipelineData?.pagination?.total;
  const statsTotal = Number.isFinite(Number(pipelineData?.stats?.total_leads))
    ? Math.max(0, Number(pipelineData?.stats?.total_leads))
    : undefined;
  const authoritativeTotal = paginationTotal ?? statsTotal ?? (offset + contacts.length);
  ...
  const stageCounts = search
    ...
    : Object.keys(backendStageCounts).length
      ...
      : visibleStageCounts;
  const counts = mergeCountsWithBackend(contacts, pipelineData?.stats, authoritativeTotal);
