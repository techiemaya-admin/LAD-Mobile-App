...
const isLocalProxyUrl = (url: string) => /^https?:\/\/(localhost|127\.0\.0\.1)(:|\/|$)/i.test(url);

const isProxyReachabilityMessage = (message: string) => {
  const lower = message.toLowerCase();
  return (
    lower.includes('unable to reach the configured backend') ||
    lower.includes('unable to reach the local lad api proxy') ||
    lower.includes('backend request timed out') ||
  ...
  for (const baseUrl of getLoginBaseUrls()) {
    try {
      ...
      if (!response.ok) {
        ...
        if (
          Platform.OS === 'web' &&
          isLocalProxyUrl(baseUrl) &&
          RETRYABLE_PROXY_STATUSES.has(response.status) &&
          isProxyReachabilityMessage(message)
        ) {
          lastNetworkError = new TypeError(message);
          continue;
        }
