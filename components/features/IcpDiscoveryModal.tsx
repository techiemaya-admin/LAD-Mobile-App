...
export function IcpDiscoveryModal({ visible, onClose, onProfileChange }: IcpDiscoveryModalProps) {
  ...
  useEffect(() => {
    if (!visible || profileLoaded) return;
    let cancelled = false;
    (async () => {
      try {
        ...
        // Profile is optional context — the conversation still works without it.
      } finally {
  ...
  }, [visible, profileLoaded, onProfileChange]);
          ...
          <View style={styles.chipWrap}>
            {(card.options || []).map((option) => {
              ...
              return (
                ...
                >
                  <Typography variant="caption" color={selected ? '#FFFFFF' : appTheme.text}>{option}</Typography>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          <View>
            <View style={styles.chipWrap}>
              {(Array.isArray(fieldVal) ? fieldVal : []).map((tag: string, index: number) => (
                <View key={`${tag}-${index}`} style={[styles.tagPill, { backgroundColor: appTheme.darkMode ? appTheme.infoSoft : '#DCE3F5' }]}>
