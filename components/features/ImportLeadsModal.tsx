...
import { extractLeadsFromFileUpload, isVisionExtractionFile } from '@/src/services/mobileAIAssistantService';
...
export function ImportLeadsModal({
  ...
  const handlePickSpreadsheet = useCallback(async () => {
    ...
    try {
      ...
      // Images and PDFs go to the backend's AI extraction endpoint instead of
      // the local spreadsheet parser, then land in the same review list.
      if (isVisionExtractionFile(asset.name || '', asset.mimeType)) {
        setExcelParsing(true);
        try {
          const extracted = await extractLeadsFromFileUpload({
            uri: asset.uri,
          ...
          if (!extracted.length) {
            setExcelError('No leads found in that file. Use a clear image of a business card or contact list.');
            return;
          }
          setLeads(extracted.map((lead) => ({
            ...newLead(),
            name: lead.name === 'Imported lead' ? '' : lead.name,
            phone: lead.phone || '',
            email: lead.email || '',
          ...
          setExcelError(extractError instanceof Error && extractError.message
            ? extractError.message
            : 'Could not extract leads from that file. Try a clearer image.');
        } finally {
  ...
  const renderResult = () => {
    ...
    if ('failedMessage' in importResult) {
      return (
        ...
        <View style={[styles.resultBadge, { backgroundColor: appTheme.darkMode ? appTheme.labelBackground : appTheme.errorSoft }]}>
    ...
    return (
      <ScrollView contentContainerStyle={{ paddingVertical: 28, paddingHorizontal: 24, gap: 14, alignItems: 'center' }}>
        <View style={[styles.resultBadge, { backgroundColor: appTheme.darkMode ? appTheme.labelBackground : appTheme.successSoft }]}>
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      {/* behavior="padding" on BOTH platforms: Android edge-to-edge (forced by
          Expo SDK 54) disables adjustResize, so without this the lead form
          fields hide behind the keyboard on device builds. */}
      <KeyboardAvoidingView
        behavior="padding"
      ...
      >
        <View style={[styles.sheet, { backgroundColor: appTheme.surface, borderColor: appTheme.borderSoft }]}>
          ...
          <View style={[styles.sheetHeader, { borderBottomColor: appTheme.borderSoft }]}>
            <View style={[styles.headerBadge, { backgroundColor: dark ? appTheme.labelBackground : '#FFF7ED', borderColor: dark ? appTheme.labelBorder : '#FFEDD5' }]}>
          ...
          {importResult ? (
            ...
            <>
              ...
              <View style={{ paddingHorizontal: 16, paddingTop: 14 }}>
                <View style={[styles.tabsTrack, { backgroundColor: dark ? appTheme.labelBackground : '#EFF6FF', borderColor: dark ? appTheme.labelBorder : '#BFDBFE' }]}>
                  {tabs.map(({ id, label, icon: Icon }) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[styles.tabPill, active && { backgroundColor: dark ? appTheme.labelBackgroundActive : '#FFFFFF', borderColor: dark ? appTheme.labelBorder : '#3B82F6', borderWidth: 1, ...Theme.shadows.small }]}
                        activeOpacity={0.8}
                      >
                ...
                <ScrollView contentContainerStyle={{ padding: 16, gap: 14 }}>
                  ...
                  >
                    ...
                    <Typography variant="body" color={appTheme.text} style={{ fontWeight: '700', marginTop: 10 }}>
                      Upload Excel, CSV, image or PDF
                    </Typography>
                    <Typography variant="caption" color={appTheme.muted} style={{ textAlign: 'center', marginTop: 4, lineHeight: 17 }}>
                      Spreadsheets need a <Typography variant="caption" color={appTheme.text} style={{ fontWeight: '700' }}>name</Typography> column
                      (optional: phone, email, company, linkedin, instagram, source).
                      {'\n'}Images and PDFs — like business cards or contact lists — are read by AI.
                    </Typography>
                    <TouchableOpacity
