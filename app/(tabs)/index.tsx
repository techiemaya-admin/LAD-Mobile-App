...
import { DashboardSection, type DashboardCard } from '@/components/features/DashboardSection';
...
export default function HomeDashboard() {
  ...
  useFocusEffect(
    useCallback(() => {
      // Returning home ends an AI Assistant session. Clear both the in-memory
      // and persisted assistant state so reopening starts with fresh data.
      useAdvancedSearchStore.getState().resetConversation();
  ...
  const openSection = (section: HomeDashboardSection, card: DashboardCard) => {
    if (section.channel === 'linkedin') {
      if (card.label === 'Generate Leads' || card.label === 'Generated Leads') {
            ...
            return (
              ...
              >
                <DashboardSection
                  ...
                  onCardPress={(card) => openSection(section, card)}
                />
              </Animated.View>
            );
          })}
