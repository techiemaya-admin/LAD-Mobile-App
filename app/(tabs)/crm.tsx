...
const getOwnerFilterValue = (contact: CrmContact) => {
  const ownerId = String(
    contact.raw.assigned_to_id
    ?? contact.raw.assignedToId
    ?? contact.raw.owner_id
    ?? contact.raw.ownerId
    ?? '',
  ).trim();
...
const buildOwnerFilterOptions = (contacts: CrmContact[]): FilterOption[] => {
  ...
  contacts.forEach((contact) => {
    ...
    owners.set(getOwnerFilterValue(contact), owner);
  });
...
const capCRMDataRows = (crmData: ProspectCRMData, limit = CRM_PAGE_SIZE): ProspectCRMData => {
...
function useCrmPalette() {
  ...
  return useMemo(() => ({
    ...
    background: appTheme.darkMode ? appTheme.background : '#F8F9FE',
    surface: appTheme.surface,
    surfaceElevated: appTheme.darkMode ? '#071131' : '#fff',
    softSurface: appTheme.darkMode ? 'rgba(30, 41, 59, 0.50)' : appTheme.softSurface,
    input: appTheme.input,
    text: appTheme.text,
    muted: appTheme.muted,
    disabled: appTheme.disabled,
    border: appTheme.darkMode ? 'rgba(148, 163, 184, 0.26)' : appTheme.border,
    borderSoft: appTheme.darkMode ? 'rgba(148, 163, 184, 0.16)' : appTheme.borderSoft,
    primary: appTheme.darkMode ? '#2976F4' : T.primary,
    primaryText: appTheme.darkMode ? '#F8FAFC' : T.primaryHead,
    icon: appTheme.darkMode ? '#E2E8F0' : '#1e293b',
    iconAccent: appTheme.darkMode ? '#7F72F9' : T.primary,
    stageActiveBackground: appTheme.darkMode ? '#0B1957' : T.primary,
    stageActiveBorder: appTheme.darkMode ? '#2976F4' : T.primary,
    stageActiveText: '#FFFFFF',
    badgeBg: appTheme.darkMode ? appTheme.labelBackground : T.badgeBg,
    neutralPill: appTheme.darkMode ? appTheme.labelBackground : '#f1f5f9',
...
export default function CRMScreen() {
  ...
  const [currentPage, setCurrentPage] = useState(() => Math.max(1, cachedCRM?.value.page ?? 1));
  ...
  const [pageError, setPageError] = useState<{ message: string; page: number } | null>(null);
  ...
  const crmScrollRef = useRef<ScrollView>(null);
  const pageRequestRef = useRef(0);
  const previousBackendFilterKeyRef = useRef('');
  const backendFilters = useMemo(() => ({
    search: query.trim() || undefined,
    stage: stageFilter !== 'all' ? stageFilter : undefined,
    channel: channelFilter !== 'all' ? channelFilter as ChannelKey : undefined,
    assignedTo: ownerFilter !== 'all' && ownerFilter !== UNASSIGNED_OWNER_FILTER ? ownerFilter : undefined,
  }), [channelFilter, ownerFilter, query, stageFilter]);
  const backendFilterKey = JSON.stringify(backendFilters);
  ...
  useFocusEffect(
    useCallback(() => {
      crmPollRef.current = setInterval(() => {
        ...
        fetchProspectCRMData({ limit: CRM_PAGE_SIZE, offset: (currentPage - 1) * CRM_PAGE_SIZE, ...backendFilters })
          .then((next) => {
            if (requestId !== pageRequestRef.current) return;
            const refreshed = capCRMDataRows(next, CRM_PAGE_SIZE);
            setData(refreshed);
            const syncedAt = Date.now();
            setLastSyncedAt(new Date(syncedAt));
            writeScreenCache(CRM_CACHE_KEY, { data: refreshed, lastSyncedAt: syncedAt, page: currentPage });
          })
          .catch(() => { /* silent */ });
      }, 90_000);
      return () => {
  ...
  const loadCRM = useCallback(async (asRefresh = false, requestedPage = currentPage) => {
    const targetPage = Math.max(1, requestedPage);
    const requestId = pageRequestRef.current + 1;
    pageRequestRef.current = requestId;
    const isPageChange = targetPage !== currentPage;

    if (asRefresh) setRefreshing(true);
    else if (data.contacts.length === 0) setLoading(true);
    else setPageLoading(true);
    setError(null);
    setPageError(null);

    try {
      const next = await fetchProspectCRMData({
        limit: CRM_PAGE_SIZE,
        offset: (targetPage - 1) * CRM_PAGE_SIZE,
        ...backendFilters,
      });
      if (requestId !== pageRequestRef.current) return;

      const capped = capCRMDataRows(next, CRM_PAGE_SIZE);
      setData(capped);
      setCurrentPage(targetPage);
      const syncedAt = Date.now();
      setLastSyncedAt(new Date(syncedAt));
      writeScreenCache(CRM_CACHE_KEY, { data: capped, lastSyncedAt: syncedAt, page: targetPage });
      if (isPageChange) {
        requestAnimationFrame(() => crmScrollRef.current?.scrollTo({ y: 0, animated: true }));
      }
    } catch (loadError) {
      if (requestId !== pageRequestRef.current) return;
      const message = loadError instanceof Error ? loadError.message : 'Could not load prospects.';
      if (isPageChange) setPageError({ message, page: targetPage });
  ...
  }, [backendFilters, currentPage, data.contacts.length]);
  ...
  useEffect(() => {
    if (!previousBackendFilterKeyRef.current) {
      previousBackendFilterKeyRef.current = backendFilterKey;
      return undefined;
    }
    if (previousBackendFilterKeyRef.current === backendFilterKey) return undefined;
    previousBackendFilterKeyRef.current = backendFilterKey;
    const timer = setTimeout(() => void loadCRM(false, 1), 300);
    return () => clearTimeout(timer);
  }, [backendFilterKey, loadCRM]);
  ...
  const paginationTotal = Math.max(0, data.pagination?.total ?? data.counts.all);
  const totalPages = Math.max(1, Math.ceil(paginationTotal / CRM_PAGE_SIZE));
  const handlePageChange = useCallback((page: number) => {
    const targetPage = Math.min(Math.max(1, page), totalPages);
    if (targetPage === currentPage || loading || refreshing || pageLoading) return;
    void loadCRM(false, targetPage);
  }, [currentPage, loadCRM, loading, pageLoading, refreshing, totalPages]);

  useEffect(() => {
    if (!loading && !refreshing && !pageLoading && currentPage > totalPages) {
      void loadCRM(false, totalPages);
    }
  }, [currentPage, loadCRM, loading, pageLoading, refreshing, totalPages]);
        ...
        setData((current) => {
          ...
          return {
            ...
            pagination: current.pagination ? {
              ...
              totalPages: Math.max(1, Math.ceil(Math.max(0, current.pagination.total - 1) / CRM_PAGE_SIZE)),
            } : undefined,
          };
        });
      } catch (removeError) {
      ...
      <ScrollView
        ...
        refreshControl={(
          <RefreshControl
            ...
            onRefresh={() => void loadCRM(true)}
            tintColor={palette.primary}
            colors={[palette.primary]}
            progressBackgroundColor={palette.darkMode ? palette.surface : '#FFFFFF'}
          />
        )}
      ...
      >
        ...
        {!loading && paginationTotal > 0 ? (
          <CrmPaginationBar
            ...
            onRetry={() => void loadCRM(false, pageError?.page ?? currentPage)}
          />
        ) : null}
...
function StatsCards({
  ...
  const cards: Array<{ key: Exclude<VisibleView, 'board'>; title: string; value: number; Icon: IconComponent; bg: string; color: string }> = [
    { key: 'all', title: 'All Contacts', value: counts.all, Icon: Users, bg: palette.darkMode ? '#0A1957' : '#dbeafe', color: palette.iconAccent },
    { key: 'prospects', title: 'Prospects', value: counts.prospects, Icon: Sparkles, bg: palette.darkMode ? '#0A1957' : '#e0e7ff', color: palette.iconAccent },
    { key: 'leads', title: 'Leads', value: counts.leads, Icon: TrendingUp, bg: palette.darkMode ? '#0A1957' : '#e0f2fe', color: palette.darkMode ? '#38BDF8' : '#0369a1' },
    { key: 'clients', title: 'Clients', value: counts.clients, Icon: BadgeCheck, bg: palette.darkMode ? '#0A1957' : '#ecfdf5', color: '#059669' },
  ];
  ...
  const pills = VIEW_DEFS.map((definition) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.viewPill,
          compact && styles.viewPillCompact,
          active && [styles.viewPillActive, { backgroundColor: palette.darkMode ? palette.badgeBg : palette.primary, shadowColor: palette.primary }],
          isPending && { opacity: 0.7 },
        ]}
      >
        <Icon color={active ? (palette.darkMode ? palette.primary : '#fff') : palette.primaryText} size={compact ? 12 : 14} />
        <Typography variant="caption" color={active ? (palette.darkMode ? palette.primary : '#fff') : palette.primaryText} style={styles.viewPillText} numberOfLines={1}>
          {definition.label}
        </Typography>
      </TouchableOpacity>
    );
    ...
    <View style={styles.boardWrap}>
      {compact ? (
        ...
        >
          {CRM_STAGES.map((stage) => {
            ...
            const activeBackground = palette.stageActiveBackground;
            const activeBorder = palette.stageActiveBorder;
            const activeText = palette.stageActiveText;
            return (
              <TouchableOpacity
                key={stage.key}
                activeOpacity={0.78}
                ...
                style={[
                  ...
                  active && [styles.stageTabActive, { backgroundColor: activeBackground, borderColor: activeBorder, shadowColor: activeBorder }],
                ]}
              >
                <Typography
                  variant="caption"
                  color={active ? activeText : palette.primaryText}
                  style={styles.stageTabText}
                  numberOfLines={1}
                >
                  {getStageLabel(stage)}
                </Typography>
                <View style={[styles.stageTabCount, { backgroundColor: active ? 'rgba(255,255,255,0.72)' : palette.neutralPill }, active && styles.stageTabCountActive]}>
                  <Typography variant="caption" color={active ? activeBackground : palette.muted} style={styles.stageTabCountText}>
                    {count}
                  </Typography>
                </View>
              </TouchableOpacity>
      ...
      >
        {visibleStages.map((stage) => {
          ...
          return (
            <View key={stage.key} style={[styles.boardColumn, { width: columnWidth, backgroundColor: palette.surface, borderColor: palette.border }]}>
...
function KanbanCard({ lead, selected, onPress }: { lead: KanbanLead; selected: boolean; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        styles.leadCard,
        { backgroundColor: palette.darkMode ? palette.input : palette.surfaceElevated, borderColor: selected ? palette.primary : palette.border },
...
function ContactList({
  ...
  // rows.length only reflects the current page's contacts, so on later pages it
  // undercounts what's actually been paged through so far (e.g. page 2 showed
  // ~94 instead of ~200). Show the running position through the total instead,
  // matching the "Showing 101-200 of N" summary in the pagination bar below.
  const loadedCount = totalCount > 0 ? Math.min(page * pageSize, totalCount) : rows.length;
      ...
      <View style={[styles.tableHeader, { borderBottomColor: palette.borderSoft }]}>
        ...
        <View style={styles.tableActions}>
          <TouchableOpacity
            ...
            activeOpacity={0.78}
            style={[styles.tableActionButton, { borderColor: filtersOpen || activeCount > 0 ? palette.primary : palette.border, backgroundColor: filtersOpen ? (palette.badgeBg) : 'transparent' }]}
          >
...
function getPaginationItems(page: number, pageCount: number): (number | string)[] {
...
function CrmPaginationBar({
  ...
  const canPrevious = safePage > 1 && !loading;
  const canNext = safePage < pageCount && !loading;
  const items = getPaginationItems(safePage, pageCount);

  return (
    <View style={[styles.paginationShell, { backgroundColor: palette.surfaceElevated, borderColor: palette.border }]}>
      <View style={styles.paginationSummary}>
        ...
        <View style={styles.paginationPageMeta}>
      ...
      {error ? (
        <View style={[styles.paginationError, { backgroundColor: palette.errorBg, borderColor: palette.errorBorder }]} accessibilityRole="alert">
          <Typography variant="caption" color={palette.errorText} style={styles.flexText}>{error}</Typography>
          <TouchableOpacity onPress={onRetry} activeOpacity={0.78} style={[styles.retryButton, { borderColor: palette.errorBorder }]}>
      ...
      <View style={styles.paginationControls}>
        <TouchableOpacity
          ...
          accessibilityLabel="Previous CRM page"
          disabled={!canPrevious}
          onPress={() => onPageChange(safePage - 1)}
          style={[styles.paginationArrow, { borderColor: palette.border, backgroundColor: palette.softSurface }, !canPrevious && styles.paginationDisabled]}
        >
          <ChevronLeft color={canPrevious ? palette.primaryText : palette.disabled} size={17} />
        </TouchableOpacity>
        ...
        {items.map((item) => typeof item === 'number' ? (
          <TouchableOpacity
            key={item}
            accessibilityRole="button"
            accessibilityLabel={`CRM page ${item}`}
            accessibilityState={{ selected: item === safePage, disabled: Boolean(loading) }}
            disabled={loading || item === safePage}
            onPress={() => onPageChange(item)}
            style={[
              styles.paginationNumber,
              { borderColor: palette.border, backgroundColor: palette.softSurface },
              item === safePage && { borderColor: palette.primary, backgroundColor: palette.darkMode ? palette.badgeBg : palette.primary },
            ]}
          >
            <Typography
              variant="caption"
              color={item === safePage ? (palette.darkMode ? palette.primary : '#FFFFFF') : palette.primaryText}
              style={styles.paginationNumberText}
            >
              {item}
            </Typography>
          </TouchableOpacity>
        ) : (
          <Typography key={item} variant="caption" color={palette.disabled} style={styles.paginationEllipsis}>...</Typography>
        ))}

        <TouchableOpacity
          accessibilityRole="button"
          accessibilityLabel="Next CRM page"
          disabled={!canNext}
          onPress={() => onPageChange(safePage + 1)}
          style={[styles.paginationArrow, { borderColor: palette.border, backgroundColor: palette.softSurface }, !canNext && styles.paginationDisabled]}
        >
          <ChevronRight color={canNext ? palette.primaryText : palette.disabled} size={17} />
        </TouchableOpacity>
      </View>
    </View>
  );
...
function TypePill({ type }: { type: string }) {
  ...
  const palette = useCrmPalette();
  const darkTone = {
    prospect: { color: '#C7D2FE', bg: 'rgba(99, 102, 241, 0.20)', border: 'rgba(165, 180, 252, 0.42)' },
    lead: { color: '#7DD3FC', bg: 'rgba(14, 165, 233, 0.18)', border: 'rgba(125, 211, 252, 0.38)' },
    client: { color: '#86EFAC', bg: 'rgba(34, 197, 94, 0.17)', border: 'rgba(134, 239, 172, 0.36)' },
    imported: { color: '#CBD5E1', bg: 'rgba(148, 163, 184, 0.16)', border: 'rgba(203, 213, 225, 0.30)' },
    inbound: { color: '#FDE68A', bg: 'rgba(245, 158, 11, 0.17)', border: 'rgba(253, 230, 138, 0.34)' },
  }[type] ?? { color: '#CBD5E1', bg: 'rgba(148, 163, 184, 0.16)', border: 'rgba(203, 213, 225, 0.30)' };
  const tone = palette.darkMode
    ? { ...darkTone, bg: palette.badgeBg }
    : { color: meta.color, bg: meta.bg, border: `${meta.color}38` };
  return (
    <View style={[styles.statusPill, { backgroundColor: tone.bg, borderColor: tone.border, borderWidth: 1 }]}>
      <Typography variant="caption" color={tone.color} style={styles.pillText}>{meta.label}</Typography>
    </View>
  );
}
...
function StagePill({ stage }: { stage?: string }) {
  ...
  const baseColor = STAGE_COLOR[stage] || '#64748b';
  const color = palette.darkMode && baseColor === T.primary ? palette.primary : baseColor;
  return (
    <View style={[styles.statusPill, { backgroundColor: palette.darkMode ? palette.badgeBg : `${color}24`, borderColor: `${color}55`, borderWidth: 1 }]}>
...
function StatusBadge({ label, color, bg }: { label: string; color: string; bg: string }) {
  const palette = useCrmPalette();
  const darkText = label.toLowerCase().includes('quiet') ? '#F8CF63' : '#FB8DA1';
  return (
    <View style={[styles.statusPill, { backgroundColor: palette.darkMode ? palette.badgeBg : bg, borderColor: palette.darkMode ? palette.border : 'transparent', borderWidth: palette.darkMode ? 1 : 0 }]}>
      <Typography variant="caption" color={palette.darkMode ? darkText : color} style={styles.pillText}>{label}</Typography>
    </View>
  );
}
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.filterChip,
        {
          borderColor: active || hovered ? palette.primary : palette.border,
          backgroundColor: active ? (palette.darkMode ? palette.badgeBg : palette.primary) : hovered ? palette.badgeBg : palette.surfaceElevated,
          opacity: pressed ? 0.78 : 1,
        },
        active && styles.filterChipActive,
      ]}
    >
      {active ? <Check color={palette.darkMode ? palette.primary : '#fff'} size={13} /> : null}
      <Typography variant="caption" color={active ? (palette.darkMode ? palette.primary : '#fff') : palette.primaryText} style={styles.filterChipText}>{label}</Typography>
    </Pressable>
  );
}
