...
export default function NotificationsModal() {
  ...
  // Opening the panel counts as viewing the notifications. The store clears the
  // unread state immediately and persists it through the backend read endpoint.
  useEffect(() => {
    const idsToMark = notifications
      .filter((notification) => !notification.read && !markingReadIds.current.has(notification.conversationId))
      .map((notification) => notification.conversationId);

    idsToMark.forEach((id) => markingReadIds.current.add(id));
    idsToMark.forEach((id) => {
      void markConversationRead(id).catch(() => markingReadIds.current.delete(id));
    });
  ...
  const openNotification = (notification: NotificationItem) => {
    if (!notification.read) {
      void markConversationRead(notification.conversationId).catch(() => undefined);
    }
    ...
    <View style={[styles.container, { backgroundColor: appTheme.background, paddingTop: insets.top }]}>
      ...
      >
        ...
        {notifications.map((notif) => {
          ...
          const displayUnreadCount = notif.unreadCount;
